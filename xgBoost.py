...
#max_depth, min_child_weight and gamma control directly model complexity
#subsample, colsample_bytree control randomness, more robust to "noise"
#small eta , increase num_rounds
params = {'eta': 0.3, #prevents overfitting --learning_rate
          'tree_method': "hist", #tree construction algorithm
          'grow_policy': "lossguide",  #split at nodes with highest loss change
          'max_leaves': 1400,  #num of leaves
          'max_depth': 0,  #maximum depth of a tree unlimited
          'subsample': 0.9,  #use 90% for training
          'colsample_bytree': 0.7, #subsample ratio of columns when constructing each tree default = 1
          'colsample_bylevel':0.7, #subsample ratio of columns for each split, in each level default = 1
          'min_child_weight':0, # no constrain on children
          'alpha':4, # L1 regularization
          'objective': 'binary:logistic', 
          'scale_pos_weight':9, # inbalance dataset default =1 
          'eval_metric': 'auc', #use AUC
          'nthread':4, #number of threads
          'random_state': 99,  #to recompute the model again
          'silent': True}
